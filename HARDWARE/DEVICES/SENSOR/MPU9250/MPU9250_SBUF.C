...
typedef struct//the structural about JY901 modular
{
	...
	enum STATR ModularStatr;//STOP or RUNNING
	enum STATR ModularSwitch;//ON or OFF
	enum STATR CalcuEnd;//If calculate is finsh,the value can be read
}JY901Modular;
...
{
	MPU9250[JY901_N].ModularStatr = STOP;
...
{
	MPU9250[JY901_N].ModularStatr = RUNNING;
}
...
void CopeSerialData(u8 JY901_N)//This function need added in abstask.c to buliding a task if you want use JY901 modular
{
	...
	if(MPU9250[JY901_N].ModularStatr)
	{
		if(MPU9250[JY901_N].ModularSwitch)
		{
			MPU9250[JY901_N].CalcuEnd = NO;
			ucRxBuffer[ucRxCnt++] = RX1_Buffer[COM1.RX_Cnt];
